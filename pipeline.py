...
2. 篇幅控制（7 级分类）：
   ...
   - 疑问: 80~120 字
   - 失误: 120~180 字
   - 漏杀: 150~220 字（优势下错过杀棋，是高水平对局的关键转折，重点分析为什么没看到）
...
3.8 【评分变化可解释性】当 step 数据中的 explanation.diagnosis 字段非空时，说明系统自动分析了"为什么评分会这样变化"。请**必须**在你的讲解中直接引用这些分析，而不是简单复述评分数字。例如：
